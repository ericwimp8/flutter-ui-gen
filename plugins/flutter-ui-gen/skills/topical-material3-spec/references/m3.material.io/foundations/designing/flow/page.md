...
- If a keyboard shortcut is activated with a single key, provide users with a way to take at least one of these actions: (Most preferred) Remap the shortcut to include one or more non-printable keyboard keys. (Preferred) Activate the shortcut only when a relevant component is focused. (Not preferred, only use as a temporary solution) Turn off the keyboard shortcut.
